    ...
    for i in range(len(matrix)):
        for j in range(len(matrix[0])):
            if i == 0 and j == 0:
                ...
                matrix[i][j] = min([matrix[i][j - 1] + 1, matrix[i-1]
                                   [j] + 1, matrix[i - 1][j - 1] + m(s1[i-1], s2[j-1])])
    return matrix[-1][-1]
        ...
        for j in range(len(d[0])):
            if i == 0 and j == 0:
                ...
                d[i][j] = min([d[i][j-1] + 1, d[i-1][j] + 1, d[i-1]
                              [j-1] + m(s1[i-1], s2[j-1]), d[i-2][j-2] + 1])
            else:
                d[i][j] = min([d[i][j - 1] + 1, d[i-1]
                                   [j] + 1, d[i - 1][j - 1] + m(s1[i-1], s2[j-1])])
    return d[-1][-1]
...
def damerau_levenstein_rec(s1, s2):
    ...
    temp = min([damerau_levenstein_rec(s1[:-1], s2) + 1,
...
def test(s1, s2):
    ...
    print(f'Damerau-Levenstein_iter({s1}, {s2}): ',
          damerau_levenstein_iter(s1, s2))
    print(f'Damerau-Levenstein_rec({s1}, {s2}): ',
          damerau_levenstein_rec(s1, s2))
    print(f'Damerau-Levenstein_rec_cash({s1}, {s2}): ',
